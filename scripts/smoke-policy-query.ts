...
async function main() {
  ...
  async function send(text: string, tag: string) {
    const body = JSON.stringify({ object: 'whatsapp_business_account', entry: [{ id: '0', changes: [{ field: 'messages', value: {
      ...
      contacts: [{ profile: { name: 'Smoke Pol' }, wa_id: TESTER }],
      messages: [{ from: TESTER, id: `wamid.SMOKEPOL_${tag}`, timestamp: '1784668000', type: 'text', text: { body: text } }],
    } }] }] })
    ...
    console.log(`[${tag}] "${text}" -> HTTP ${res.status}`); await new Promise((r) => setTimeout(r, 3500))
  }
  await send('hola', 'CONSENT')
  await send('cual es la politica de privacidad?', 'POLICY')       // → link, NO escala
  await send('quiero eliminar mis datos', 'RIGHT')                 // → escala (derecho)
  ...
  if (conv) {
    const { data: msgs } = await supa.from('messages').select('role, content').eq('conversation_id', conv.id).order('created_at', { ascending: false }).limit(5)
    console.log('\nMensajes:')
    for (const m of (msgs ?? []).reverse()) console.log(`  [${m.role}] ${String(m.content).slice(0, 150)}`)
    console.log('\nstatus (debe ser escalated por el RIGHT):', conv.status, '| conv_id a limpiar:', conv.id)
  }
}
